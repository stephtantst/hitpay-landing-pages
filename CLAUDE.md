...
**Reliability**: `/api/generate` has `maxDuration = 800` (the GA ceiling for Fluid Compute on Vercel Pro/Enterprise — raised from an original 300s because genuinely slow-but-healthy generations, not stalled ones, were being killed mid-stream). Vercel still hard-kills the function at that point with no chance for app code to run (no error event, no DB update), which can leave a brief stuck at `status: 'generating'` forever with the page never saved. Hardening in place:
...
- `app/new/page.tsx` has its own client-side backstop — a 90s no-data stall timer plus an absolute ~14.2-minute ceiling (matching the 800s server limit) — since a Vercel hard-kill can leave the connection silently dead with no error event ever arriving.
- **Per-generation cost cap**: `max_tokens` for each attempt is sized off a shared `MAX_GENERATION_COST_USD` (0.50) budget in `lib/anthropic.ts`, estimated from the actual prompt content rather than hardcoded — a stall-triggered retry gets whatever's left of that budget after subtracting the failed attempt's estimated spend, and skips the retry entirely once the remaining budget drops below `MIN_RETRY_BUDGET_USD` (0.05). This bounds total spend per user-initiated generation (including its one retry) independently of `maxDuration` — raising the duration ceiling lets slow-but-bounded-cost generations finish; it doesn't raise the cost ceiling.
...
The refine flow (`app/api/pages/[id]/refine/route.ts`, also `maxDuration = 800`) gets the same treatment: `refineHtml`'s full-regen fallback has the same stall-detect-and-retry and cost cap as `generateHtml`; `proposeEdits`' non-streaming call gets a capped request timeout (a timeout there is just treated as a normal failure, since the route already falls back to `refineHtml` on any edit-proposal error); and `app/pages/[id]/page.tsx`'s refine handler has the same 90s-stall/~14.2min-ceiling client-side backstop as `app/new/page.tsx`.
