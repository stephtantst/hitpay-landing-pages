...
Note: a genuinely slow (not stalled) generation that exceeds 300s still gets hard-killed on this plan — the cost cap and stall-guard don't fix that specific failure mode, only an actual Pro-plan upgrade (raising `maxDuration` toward 800s) would.
