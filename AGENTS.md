...
3. **HTML generation** (`lib/anthropic.ts: generateHtml`) → Codex Sonnet streams HTML; system prompt and research context are prompt-cached
...
- `MAX_CONTINUATION_ROUNDS` (6) bounds how many times this can happen; hitting the cap without Codex ever signaling `end_turn` still saves whatever was generated (flagged as possibly incomplete) rather than discarding real, already-paid-for output.
